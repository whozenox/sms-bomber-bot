#!/usr/bin/env python
# ULTIMATE BOMBER BOT - ADMIN BUTTON IN UI
...
def create_user(user_id, username, first_name, referral_code=None):
    ...
    c.execute('''INSERT INTO users (user_id, username, first_name, coins, referral_code, referred_by, join_date)
        VALUES (?, ?, ?, ?, ?, ?, ?)''',
        (user_id, username or first_name, first_name, FREE_COINS, ref_code, referred_by, datetime.now().isoformat()))
    if referred_by:
