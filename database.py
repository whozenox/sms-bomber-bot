# database.py
# ULTIMATE SMS BOMBER BOT - FINAL DATABASE
...
def add_bomb_stats(user_id, target, sms_count, success, failed, cost, speed):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE users SET total_bombs = total_bombs + 1, total_sms = total_sms + ? WHERE user_id = ?", 
              (sms_count, user_id))
    c.execute('''INSERT INTO bomb_logs (user_id, target, sms_count, success, failed, cost, speed, date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (user_id, target, sms_count, success, failed, cost, speed, datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def get_bomb_logs(user_id, limit=10):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT target, sms_count, success, failed, cost, speed, date FROM bomb_logs WHERE user_id = ? ORDER BY date DESC LIMIT ?", 
              (user_id, limit))
    logs = c.fetchall()
    conn.close()
    return logs
