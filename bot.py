# bot.py
# ULTIMATE SMS BOMBER BOT - COMPLETE FINAL
...
async def process_bomb(update, phone, count, speed, context):
    user_id = update.effective_user.id
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🐢 SLOW - Stable", callback_data="speed_slow"), InlineKeyboardButton("⚡ MEDIUM - Balanced", callback_data="speed_medium")],
...
async def get_speed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🎯 200 SMS - 2💰", callback_data="count_200"), InlineKeyboardButton("💥 500 SMS - 5💰", callback_data="count_500")],
...
async def get_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ CONFIRM & BOMB", callback_data="confirm_bomb"), InlineKeyboardButton("🔙 Back", callback_data="back_to_count")]
    ]
...
async def confirm_bomb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create fake update for process_bomb
    class FakeMessage:
        async def reply_text(self, text, *args, **kwargs):
            await query.edit_message_text(text, *args, **kwargs)
            return self
    ...
    await process_bomb(fake_update, phone, count, speed, context)
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if hasattr(update, 'callback_query'):
        query = update.callback_query
        await query.answer()
        await query.edit_message_text("❌ **Cancelled!**\n\nUse /bomb to start again.", parse_mode='Markdown')
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type in ['group', 'supergroup']:
        await update.message.reply_text("🤖 **Bot is active in this group!**", parse_mode='Markdown')
        return
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in active_bombs and active_bombs[user_id]['active']:
        ...
        await update.message.reply_text("❌ **No active bombing session found!**", parse_mode='Markdown')
...
"""
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⚡ Speed", callback_data="speed_menu"), InlineKeyboardButton("🛑 Stop", callback_data="stop")],
        [InlineKeyboardButton("💰 Coins", callback_data="credits"), InlineKeyboardButton("👥 Refer", callback_data="refer")],
        [InlineKeyboardButton("📊 Stats", callback_data="stats"), InlineKeyboardButton("📋 History", callback_data="history")],
        [InlineKeyboardButton("💳 Buy", callback_data="buy"), InlineKeyboardButton("🔄 Transfer", callback_data="transfer")],
        [InlineKeyboardButton("🎯 Redeem", callback_data="redeem"), InlineKeyboardButton("ℹ️ Info", callback_data="info")],
        [InlineKeyboardButton("📢 Channel 1", url="https://t.me/zenoxtool"), InlineKeyboardButton("📢 Channel 2", url="https://t.me/Dev_Null_X_NODE_JS")],
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton(f"{SPEED_SETTINGS['slow']['label']} - {SPEED_SETTINGS['slow']['description']}" + (" ✅" if current_speed == 'slow' else ""), callback_data="speed_slow")],
        [InlineKeyboardButton(f"{SPEED_SETTINGS['medium']['label']} - {SPEED_SETTINGS['medium']['description']}" + (" ✅" if current_speed == 'medium' else ""), callback_data="speed_medium")],
        [InlineKeyboardButton(f"{SPEED_SETTINGS['fast']['label']} - {SPEED_SETTINGS['fast']['description']}" + (" ✅" if current_speed == 'fast' else ""), callback_data="speed_fast")],
...
"""
    ...
    keyboard = [[InlineKeyboardButton("📤 Share Link", url=f"https://t.me/share/url?url={link}&text=🔥%20Join%20the%20Ultimate%20SMS%20Bomber%20Bot!%20Get%20{REFERRAL_BONUS}%20free%20coins!")]]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "send_sms":
        ...
        await query.edit_message_text("🐢 **Speed set to SLOW!**\n\n✅ Stable & Safe\n📱 200 SMS → 10-15 seconds\n\nUse /speed to change anytime.", parse_mode='Markdown')
        return
    
    elif data == "speed_medium":
        db.set_user_speed(user_id, 'medium')
        await query.edit_message_text("⚡ **Speed set to MEDIUM!**\n\n✅ Balanced Speed\n📱 200 SMS → 4-6 seconds\n\nUse /speed to change anytime.", parse_mode='Markdown')
        return
    
    elif data == "speed_fast":
        db.set_user_speed(user_id, 'fast')
        await query.edit_message_text("🚀 **Speed set to FAST!**\n\n✅ Maximum Speed\n📱 200 SMS → 1-2 seconds\n\nUse /speed to change anytime.", parse_mode='Markdown')
        return
    
    elif data == "stop":
        if user_id in active_bombs and active_bombs[user_id]['active']:
            active_bombs[user_id]['active'] = False
            await query.edit_message_text(f"🛑 **Bombing Stopped!**\n\n📱 Target: `{active_bombs[user_id]['phone']}`\n📊 Remaining: {active_bombs[user_id]['count']} SMS\n\n💡 Use /bomb to start again.", parse_mode='Markdown')
        else:
            await query.edit_message_text("❌ **No active bombing session found!**", parse_mode='Markdown')
        return
        ...
        bot_username = bot_info.username
        link = f"https://t.me/{bot_username}?start={code}"
        await query.edit_message_text(f"👥 **REFERRAL SYSTEM**\n\nYour Code: `{code}`\nYour Link: {link}\n\n💰 You get +{REFERRAL_BONUS} coins per referral!\n📊 Total: {db.get_referral_count(user_id)} referrals", parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Share Link", url=f"https://t.me/share/url?url={link}&text=🔥%20Join%20the%20Ultimate%20SMS%20Bomber%20Bot!%20Get%20{REFERRAL_BONUS}%20free%20coins!")]]))
        return
    
    elif data == "credits":
        bal = db.get_balance(user_id)
        stats = db.get_user_stats(user_id)
        await query.edit_message_text(f"💰 **YOUR COINS**\n\nBalance: {bal} coins\nTotal Bombs: {stats['total_bombs'] if stats else 0}\nSMS Sent: {stats['total_sms'] if stats else 0}\nReferrals: {db.get_referral_count(user_id)}\nTotal Spent: {stats['total_spent'] if stats else 0}\n\n💡 /buy - Purchase more coins\n👥 /refer - Get free coins")
        return
        ...
        for i, log in enumerate(logs, 1):
            target, sms_count, success, failed, cost, speed, stopped, date = log
            status = "🛑 Stopped" if stopped else "✅ Complete"
            text += f"{i}. 📱 `{target}`\n   ✅ {success} ❌ {failed} | {sms_count} SMS | {cost} coins\n   ⚡ {SPEED_SETTINGS.get(speed, SPEED_SETTINGS['slow'])['label']} | {status} | 📅 {date[:10]}\n\n"
        ...
        await query.edit_message_text("🔄 **TRANSFER COINS**\n\nUsage: `/transfer <user_id> <amount>`\n\nExample: `/transfer 123456789 50`\n\n⚠️ You can only transfer to existing users!", parse_mode='Markdown')
        return
    
    elif data == "redeem":
        await query.edit_message_text("🎯 **REDEEM CODE**\n\nUsage: `/redeem <code>`\n\nExample: `/redeem BOMBER2024`\n\n📌 Contact admin for redeem codes!", parse_mode='Markdown')
        return
    
    elif data == "info":
        await query.edit_message_text(f"ℹ️ **{BOT_NAME} INFO**\n\n📅 Version: {VERSION}\n👑 Owner: {OWNER}\n📢 Channels: {CHANNEL1} | {CHANNEL2}\n📡 APIs: {len(APIS)}\n💰 Pricing: 200→2 | 500→5 | ∞→8\n🎁 Free: {FREE_COINS} coins\n👥 Referral: +{REFERRAL_BONUS} coins\n⏰ Auto-Stop: {AUTO_STOP_MINUTES} min\n\n📌 Made with ❤️ by {OWNER}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📢 Channel 1", url="https://t.me/zenoxtool"), InlineKeyboardButton("📢 Channel 2", url="https://t.me/Dev_Null_X_NODE_JS")], [InlineKeyboardButton("👑 Owner", url="https://t.me/lordzenox")]]))
        return
        ...
        total_bombs = db.get_total_bombs_used()
        total_sms = db.get_total_sms_sent_all()
        await query.edit_message_text(f"📊 **FULL STATS**\n\n👥 Users: {stats['users']}\n🔥 Active Bombers: {active_users}\n💣 Total Bombs: {total_bombs}\n📱 Total SMS: {total_sms}\n💰 Coins: {stats['coins']}\n👑 Admins: {stats['admins']}\n🚫 Banned: {stats['banned']}\n📡 APIs: {len(APIS)}")
        return
